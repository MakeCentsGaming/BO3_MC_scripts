...
#using scripts\zm\mc_dyn_zones\mc_dyn_zones;
Comment out the following:
    // level.zone_manager_init_func =&usermap_test_zone_init;
Add the following under the previous:
    level.zone_manager_init_func =&mc_dyn_zones::SetUpZones;

if using dyn doors change it to the following:
	level.zones = [];
	// level.zone_manager_init_func =&usermap_test_zone_init;
	
	init_zones[0] = "start_zone";
	foreach(zone in init_zones)
	{
		zm_zonemgr::zone_init( zone );
...
Add to zone file
scriptparsetree,scripts/zm/mc_dyn_zones/mc_dyn_zones.gsc
...
function private CheckTriggers()
{
...
function private DeleteZoneStructs()
{
	foreach(z in level.zonestructs)
	{
		struct::delete();
...
function private DelayPrint(z, index = 0)
{
	wait(index+12);
	IPrintLn(z);
}

function private SpawnModels()
{	
...
function private AddFlagToEachTarget()
{
...
function private AddThisFlag(dzs)
{
...
function private GetClosestTriggerScriptFlag(max = 100000)
{
...
function private GetClosestTrigger(trigs, max = 100000)
{	
...
function private GetUseTrigsWithScriptFlags()
{
	trigs =getentarray("trigger_use", "classname");
	// IPrintLnBold(trigs.size + " available trigs");
	strigs= [];
...
function private GetAdjacentZones()
{
	adjacentzones= [];
...
function private AddAdjacentPrint(a,b,f,time)
{
	wait(20+time);
